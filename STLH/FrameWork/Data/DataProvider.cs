...
namespace FrameWork.Data
{
    ...
    public abstract class DataProvider
    {
        ...
        protected object Populatesys_User(IDataReader dr)
        {
            ...
            if (!Convert.IsDBNull(dr["U_LoginType"])) nc.U_LoginType = Convert.ToString(dr["U_LoginType"]).Trim(); // 登入类型(为空系统认证,其它值为其它认证)
            return nc;
        }
        #endregion
        ...
        protected object Populatesys_Module(IDataReader dr)
        {
            ...
            if (!Convert.IsDBNull(dr["M_Icon"])) nc.M_Icon = Convert.ToString(dr["M_Icon"]).Trim(); // 模块Icon
            if (!Convert.IsDBNull(dr["M_Info"])) nc.M_Info = Convert.ToString(dr["M_Info"]).Trim(); // 模块说明
            return nc;
        }
        #endregion
