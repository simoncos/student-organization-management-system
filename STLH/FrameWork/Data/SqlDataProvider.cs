    ...
    public class SqlDataProvider : DataProvider
    {
        ...
        public override int sys_ModuleInsertUpdate(sys_ModuleTable fam)
        {
            ...
            using (SqlConnection Conn = GetSqlConnection())
            {
                ...
                cmd.Parameters.Add("@M_Icon", SqlDbType.VarChar).Value = fam.M_Icon;  //模块Icon
                cmd.Parameters.Add("@M_Info", SqlDbType.NVarChar).Value = fam.M_Info;  //模块说明
                Conn.Open();
                rInt = Convert.ToInt32(cmd.ExecuteScalar());
                cmd.Dispose();
                Conn.Dispose();
        ...
        public override int sys_UserInsertUpdate(sys_UserTable fam)
        {
            ...
            using (SqlConnection Conn = GetSqlConnection())
            {
                ...
                cmd.Parameters.Add("@U_LoginType",SqlDbType.VarChar).Value = fam.U_LoginType;  //登入类型(为空系统认证,其它值为其它认证)
                Conn.Open();
                rInt = Convert.ToInt32(cmd.ExecuteScalar());
                cmd.Dispose();
                Conn.Dispose();
        ...
        public override int Update_Table_Fileds(string Table, string Table_FiledsValue, string Wheres)
        {
            ...
            using (SqlConnection Conn = GetSqlConnection())
            {
                string strSql = string.Format("Update {0} Set {1}  Where {2};select @@rowcount ", Table, Table_FiledsValue, Wheres);
                SqlCommand cmd = new SqlCommand(strSql, Conn);
                cmd.CommandType = CommandType.Text;
                Conn.Open();
                rInt = Convert.ToInt32(cmd.ExecuteScalar());
