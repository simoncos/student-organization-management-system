...
namespace FrameWork
{
    ...
    public class FrameWorkPermission : IHttpModule
    {
        ...
        {            
            //判断是否在manager目录里
            if (Common.GetCharInStringCount("/", Common.GetScriptUrl.ToLower()) > 0)
            {
        ...
        private void app_AuthMethod(object sender, EventArgs e)
        {
            ...
            if (Common.GetScriptNameExt.ToLower() == "aspx" && Common.GetCharInStringCount("/", Common.GetScriptUrl.ToLower()) > 0)
            {
                //检测方法权限设置
